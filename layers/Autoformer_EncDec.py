...
class series_decomp_multi(nn.Module):
    ...
    def forward(self, x):
        ...
        # project each feature value to a vector of size=kernel_size
        # apply Softmax along kernel_size dim
...
class EncoderLayer(nn.Module):
    ...
    def __init__(
        self,
        attention,  # AutoCorrellationLayer instance
        d_model,
        d_ff=None,
        moving_avg=25,
        dropout=0.1,
        ...
        # 2 conv1d run consecutively
        self.conv1 = nn.Conv1d(in_channels=d_model, out_channels=d_ff, kernel_size=1, bias=False)
        self.conv2 = nn.Conv1d(in_channels=d_ff, out_channels=d_model, kernel_size=1, bias=False)

        # init mean decomposition layers
    ...
    def forward(self, x, attn_mask=None):
        # apply autocorrelation layer
        new_x, attn = self.attention(
            x, x, x,
            attn_mask=attn_mask,
        )
        # first skipped connection
        # NB: we apply dropout after we computed
        # new token embeddings
        x = x + self.dropout(new_x)

        # subtrace moving average
        x, _ = self.decomp1(x)

        # apply conv1d two times
        y = x
        y = self.dropout(self.activation(self.conv1(y.transpose(-1, 1))))
        y = self.dropout(self.conv2(y).transpose(-1, 1))

        # combine outputs;
        # rm moving average second time
